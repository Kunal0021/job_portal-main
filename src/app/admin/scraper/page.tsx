...
export default function ScraperManager() {
  ...
  const handleEditJob = async (companyId: string) => {
    try {
      setLoading(true);
      // Fetch the latest job for this company
      ...
      if (error) {
        if (error.code === 'PGRST116') {
          alert('No jobs found for this company to edit.');
        } else {
          throw error;
        }
        return;
  ...
  const handleSaveJob = async (updatedData: any) => {
    setSavingJob(true);
    try {
      const { id, created_at, companies, ...updatePayload } = updatedData;
  ...
  if (isEditing && editingJob) {
    return (
      <div className="min-h-screen bg-[#F8FAFC] pb-20 p-6 md:p-10">
        <div className="max-w-6xl mx-auto">
          <JobForm 
            ...
            loading={savingJob}
            title="Edit Scraped Job"
            subtitle={`Refining the latest posting for ${allCompanies.find(c => c.id === editingJob.company_id)?.name || 'Company'}`}
          />
        </div>
      </div>
    );
                        ...
                        <td className="px-8 py-5 text-right">
                          <div className="flex items-center justify-end gap-2">
                            <Button 
                              variant="ghost" 
                              size="icon" 
                              className="h-9 w-9 rounded-xl text-gray-400 hover:text-indigo-600 hover:bg-indigo-50 shadow-sm shadow-indigo-100/20 border border-indigo-50" 
                              title="Edit Latest Job" 
                              onClick={() => handleEditJob(company.id)}
                            >
                  ...
                  <tbody className="divide-y divide-gray-50 bg-white">
                    {filteredLogs.map((log) => (
                      <tr key={log.id} className="hover:bg-indigo-50/20 transition-all group">
                        ...
                        <td className="px-8 py-5 text-right">
                           <div className="flex items-center justify-end gap-2">
                              <Button variant="ghost" size="icon" className="h-9 w-9 rounded-xl text-gray-400 hover:text-indigo-600 hover:bg-indigo-50" title="Edit" onClick={() => alert('Edit action triggered')}>
                                <Edit className="h-4 w-4" />
                              </Button>
