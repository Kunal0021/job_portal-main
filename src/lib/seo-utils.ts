...
export function calculateSEOScore(job: any): SEOResult {
  ...
  // Helper to count words (stripping HTML first)
  const stripHtml = (html: string) => html.replace(/<[^>]*>?/gm, ' ').replace(/\s+/g, ' ').trim();
  const plainText = stripHtml(content);
  
  const wordCount = plainText.split(/\s+/).filter(Boolean).length;
  const keywordCount = focusKeyword ? (plainText.match(new RegExp(focusKeyword, 'gi')) || []).length : 0;
  ...
  const keywordInTitleStart = hasKeyword && titleWords.includes(focusKeyword.toLowerCase().trim());
  checks.push({
    id: 2,
    name: 'Keyword in SEO Title (first 3 words)',
    points: 5,
    passed: keywordInTitleStart,
    message: keywordInTitleStart ? 'Keyword is at the start of title.' : 'Keyword not found in the first 3 words of title.',
    category: 'title',
  ...
  const keywordInTitleEntirely = hasKeyword && title.toLowerCase().includes(focusKeyword.toLowerCase().trim());
  checks.push({
    id: 11,
    name: 'Title contains Focus Keyword',
    points: 10,
    passed: keywordInTitleEntirely,
    message: keywordInTitleEntirely ? 'Keyword found in title.' : 'Full focus keyword missing from title. Rewrite title to include it.',
    category: 'title',
    autoFixAvailable: true
  });
  ...
  const keywordInSubheading = hasKeyword && (content.match(/<h[23][^>]*>([\s\S]*?)<\/h[23]>/gi) || []).some((h: string) => h.toLowerCase().includes(focusKeyword.toLowerCase()));
  checks.push({
    id: 6,
  ...
  checks.push({
    id: 21,
    name: 'Word count 400-2,500',
    points: 5,
    passed: wordCount >= 400 && wordCount <= 2500,
    message: `Content is ${wordCount} words. Target 400-2,500.`,
    category: 'content',
    autoFixAvailable: true
  });
  ...
  const hasDensity = density >= 0.5 && density <= 2.0;
  checks.push({
    id: 7,
    name: 'Keyword Density 0.5%-2.0%',
    points: 1,
    passed: hasDensity,
    message: `Density is ${density.toFixed(2)}%. Target 0.5%-2.0%.`,
    category: 'keyword',
    autoFixAvailable: true
  });
