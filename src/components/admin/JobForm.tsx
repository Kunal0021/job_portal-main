...
export function JobForm({ 
  ...
  const [isSuperOptimizing, setIsSuperOptimizing] = useState(false);
...
Return ONLY the new meta description. No explanation.`
      ...
      } else if (check.category === 'content' || check.category === 'keyword') {
        const response = await fetch('https://api.openai.com/v1/chat/completions', {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
          ...
          body: JSON.stringify({
            ...
            messages: [
              {
                role: 'system',
                content: `You are an expert SEO editor. Fix this specific issue: "${check.name}: ${check.message}".
Focus Keyword: ${currentJob.focus_keyword}

STRICT RULES:
1. ONLY fix the specific issue mentioned. Preserve all other content exactly.
2. If fixing FAQ/TOC, append it naturally.
3. If fixing Keyword Density/Word Count, refine the existing text.
4. Return ONLY the new HTML content. No conversational text.`
              },
              {
                role: 'user',
                content: currentJob.content_html || currentJob.description
              }
            ]
          })
        });
  ...
  const handleSuperAIOptimize = async () => {
    if (!currentJob.description && !currentJob.content_html) {
      alert('Please provide some job content first.');
      return;
    }

    setIsSuperOptimizing(true);
    try {
      // Step 1: Initialize best version tracking
      let bestJobVersion = { 
        ...currentJob,
        focus_keyword: currentJob.focus_keyword || generateFocusKeyword(currentJob.title, currentJob.location)
      };
      let bestScore = calculateSEOScore(bestJobVersion).score;
      let optimizedJob = { ...bestJobVersion };
      ...
      while (attempts < maxAttempts) {
        attempts++;
        const report = calculateSEOScore(optimizedJob);
        const failingChecks = report.checks.filter(c => !c.passed);

        // Terminate if 100% score reached
        if (failingChecks.length === 0 || report.score === 100) break;
        ...
        const response = await fetch('https://api.openai.com/v1/chat/completions', {
          ...
          body: JSON.stringify({
            ...
            messages: [
              {
                role: 'system',
                content: `You are a Systematic SEO Auditor and Fixer. YOUR MISSION: Reach a perfect 100/100 SEO score.
                
MISSION PRIORITIES (High Points):
- Foundations: Focus Keyword Set, Title Keyword, Meta Keyword, URL Slug, and H1 Tag (Target: 80+ score immediately).
- Perfection: Full Title/Meta rules and all Content structural items (Target: 100/100).

STRICT REPAIR GUIDELINES:
1. Focus Keyword: "${optimizedJob.focus_keyword}" (Use this exactly).
2. Fix SEO Title: STRICTLY 50-60 chars, must start with keyword, include a Power Word (Urgent/Immediate/Verified) and a Sentiment Word.
3. Fix Meta Description: 130-160 chars, include keyword, a Call-to-Action, and a SALARY signal (e.g., LPA, ₹, $, or "Competitive Salary").
4. Enhance Content HTML:
   - Inject H1 with keyword at the start.
   - Insert a "Table of Contents" section.
   - Ensure at least 3 distinct H2 subheadings.
   - Ensure at least one H2 or H3 subheading contains the focus keyword precisely.
   - Include at least 2 internal links (e.g., <a href="/jobs">Explore Jobs</a>).
   - Include at least 1 external link (e.g., <a href="https://google.com">industry standards</a>).
   - Add an 'FAQ' section with at least 2 questions.
   - Word count target: 900-1100 words.
   - Keyword density: Target EXACTLY 1% (repeat focus keyword naturally ~10 times per 1000 words).
   - Readability: Use short paragraphs (strictly max 3 sentences each).

${attempts > 1 ? `SYSTEM NOTICE: PREVIOUS ATTEMPT WAS INCOMPLETE. Score: ${calculateSEOScore(optimizedJob).score}/100.` : ''}
SYSTEMATIC LIST OF CRITERIA TO VERIFY AND FULFILL:
...
Output JSON format: { 
    ...
    "meta_description": "string", 
    "url_slug": "string", 
    "content_html": "string (Optimized and Verified HTML)"
}`
              },
              {
                role: 'user',
                content: `Job Title: ${optimizedJob.title}, Company: ${companies.find(c => c.id === optimizedJob.company_id)?.name || 'Gethyrd'}, Location: ${optimizedJob.location}. Job Description: ${optimizedJob.content_html || optimizedJob.description}`
              }
            ]
          })
        });
        ...
        const newVersion = {
          ...
          url_slug: aiResponse.url_slug || optimizedJob.url_slug,
          description: aiResponse.content_html,
          content_html: aiResponse.content_html,
          focus_keyword: optimizedJob.focus_keyword
        };

        const newScore = calculateSEOScore(newVersion).score;
        if (newScore > bestScore) {
          bestScore = newScore;
          bestJobVersion = newVersion;
        }

        // Continue refinement with the newest version to allow AI to fix its own errors
        optimizedJob = newVersion;

        // Re-verify after this pass
        if (newScore === 100) break;
      }

      setCurrentJob(bestJobVersion);
      
      if (bestScore === 100) {
        alert('🎯 All SEO criteria verified and fulfilled! Score: 100/100.');
      } else {
        alert(`Verification complete. Best achieved Score: ${bestScore}/100. (The best version was preserved).`);
      }
    } catch (error) {
      console.error('Super Optimize Error:', error);
      alert('Verification failed. Please check your network or API settings.');
    } finally {
      setIsSuperOptimizing(false);
    }
  };

  const handleEnhanceDescription = async () => {
    if (!currentJob.description) {
      alert('Please provide a job description first before enhancing.');
      return;
    }

    if (!process.env.NEXT_PUBLIC_OPENAI_API_KEY) {
      alert('OpenAI API Key is missing. Please check your .env file and restart the server.');
      return;
    }
                ...
                <CheckCircle2 className="h-5 w-5 text-emerald-500" /> SEO Checklist
                  ...
                  {seoReport.score < 100 && (
                    <Button 
                      size="sm" 
                      variant="ghost" 
                      disabled={isSuperOptimizing}
                      onClick={handleSuperAIOptimize}
                      className="h-7 px-2 text-[9px] font-black bg-indigo-600 text-white hover:bg-indigo-700 rounded-lg flex items-center gap-1 shadow-sm transition-all active:scale-95"
                    >
                      {isSuperOptimizing ? <Loader2 className="h-3 w-3 animate-spin" /> : <Sparkles className="h-3 w-3" />}
                      SUPER AI OPTIMIZE
                    </Button>
                  )}
            </div>
